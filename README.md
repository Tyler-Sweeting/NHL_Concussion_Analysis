...
If we were to do furthur analysis on this project, there are a few things that could be done next. We could try to find some datasets from further back in time to compare the impact concussions had back then to the impact they have now. For our machine learning model we needed to create two additional columns of data, being points-per-game in the previous season and in the following season of a concussion. When adding this to the points per game of the season of the concussion that makes for 3 columns. It could be reduced to 2 columns by changing it to difference between previous season and current season, as well as difference between following season and current season. 
